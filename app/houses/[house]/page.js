...
import { ChevronLeft, Award, Star, Trophy, Zap, Medal, AlertCircle, CheckCircle2 } from 'lucide-react';
...
const houseDescriptions = {
  'atreides': {
    description: "A house of honor and justice, whose leadership principles and strategic brilliance have earned them respect across the universe.",
    specialty: "Leadership",
    motto: "The measure of a person is what they do with power."
  },
  'arrakis': {
    description: "Born of the harsh desert, they've mastered survival in the most unforgiving landscapes, finding beauty and strength where others see only desolation.",
    specialty: "Desert Craft",
    motto: "The spice must flow."
  },
  'winterfell': {
    description: "Steadfast and resilient as the northern winters, they embody honor and tradition while preparing for the long challenges ahead.",
    specialty: "Strategy",
    motto: "Winter is coming."
  },
  'zephandor': {
    description: "Fluid and adaptable, they harness the power of innovation and knowledge, flowing around obstacles rather than confronting them directly.",
...
const HouseDetails = () => {
  ...
  useEffect(() => {
    ...
    if (houseInfo && score) {
      ...
      const processedEvents = score.scores?.map(eventScore => {
        ...
        const housesWithScores = Object.values(eventScore.scores || {})
        ...
        return {
          ...
          scorePercentage: maxScore > 0 ? (thisHouseScore / maxScore) * 100 : 0
        };
      }) || [];
  ...
  return (
    <div className={styles.houseDetails}>
      ...
      <div 
        ...
        ref={headerRef}
      >
        <div className={styles.houseColorAccent} style={{ backgroundColor: houseData.color }}></div>
        ...
        <div className={styles.houseInfo}>
          <div className={styles.logoContainer}>
            <div className={styles.logoGlow} style={{ backgroundColor: houseData.color }}></div>
            <img src={houseData.logo} alt={houseData.name} className={styles.houseLogo} />
          </div>
          ...
          <div className={styles.specialtyBadge} style={{ borderColor: houseData.color }}>
      ...
      <div className={styles.contentContainer} ref={contentRef}>
        <div className={styles.eventsContainer}>
                  ...
                  >
                    <div className={styles.eventContent}>
                      <div className={styles.eventName}>
                        {event.name}
                        {event.isHighest && (
                          <span className={styles.highestScore} title="Highest score">
                            <Star size={14} />
                          </span>
                        )}
                      </div>
                      <div className={styles.eventPosition}>
                        {event.position === 1 ? (
                          <div className={styles.positionBadge} title="1st Place">
                            <Medal size={16} color={houseData.color} />
                          </div>
                        ) : event.position === 2 ? (
                          <div className={styles.positionBadge} title="2nd Place">2</div>
                        ) : event.position === 3 ? (
                          <div className={styles.positionBadge} title="3rd Place">3</div>
                        ) : null}
                      </div>
                    </div>
                    ...
                    <div className={styles.eventScoreContainer}>
                      ...
                      <div className={styles.scoreDetails}>
                        <div className={styles.scoreComparison}>
                          <span className={styles.scoreValue} style={{ color: houseData.color }}>
                            {event.score}
                          </span>
                        </div>
                      </div>
          ...
          {houseData.participatedEvents && houseData.participatedEvents.length > 0 ? (
            <>
              <div className={styles.eventSectionTitle}>
              ...
              <div className={styles.eventsList}>
                {houseData.participatedEvents.map((event, index) => (
                  <div 
                    ...
                    style={{ animationDelay: `${(houseData.scoredEvents?.length || 0) * 0.1 + index * 0.1}s` }}
                  >
                    <div className={styles.eventContent}>
                    ...
                    <div className={styles.emptyScoreBar}>
                      <div className={styles.scoreDetails}>
                        <div className={styles.scoreComparison}>
                          <span className={styles.scoreValue} style={{ color: 'var(--text-secondary)' }}>
                            0
                          </span>
                        </div>
                      </div>
        ...
        <div className={styles.houseStats}>
          <div className={styles.statCard}>
            <div className={styles.statValue}>{houseData.allEvents?.length || 0}</div>
          ...
          <div className={styles.statCard}>
            <div className={styles.statValue}>
              {houseData.scoredEvents?.filter(event => event.isHighest)?.length || 0}
            </div>
